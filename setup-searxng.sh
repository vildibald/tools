...
# setup-searxng.sh
#
# Creates /opt/searxng with:
#   - docker-compose.yml      (SearXNG core + Valkey cache + MCP server)
#   - .env                    (runtime environment overrides)
#   - .env.example            (documented template)
#   - core-config/settings.yml (SearXNG settings with random secret key)
#   - mcp-server/             (custom MCP HTTP server: Dockerfile, code, deps)
#   - systemd unit            (auto-starts the compose stack on boot)
#
# Then enables and starts the systemd service.
#
# Services exposed after setup:
#   - SearXNG web UI  : http://localhost:8001
#   - MCP HTTP server : http://localhost:8000/mcp
#
# SECURITY NOTE: SearXNG listens on 0.0.0.0:8001 with no built-in auth.
...
if ! command -v openssl &>/dev/null; then
    echo "ERROR: 'openssl' is not installed (needed to generate secret key)."
    exit 1
fi
...
UNIT_FILE="/etc/systemd/system/searxng-stack.service"
...
# ─── Create directories ─────────────────────────────────────────────────────
echo "==> Creating SearXNG directory structure: ${SEARXNG_DIR}"
mkdir -p "${SEARXNG_DIR}/core-config"
mkdir -p "${SEARXNG_DIR}/mcp-server"

# ─── Generate secret key ────────────────────────────────────────────────────
SECRET_KEY="$(openssl rand -hex 32)"
echo "==> Generated new secret key for SearXNG server"

# ─── docker-compose.yml ─────────────────────────────────────────────────────
echo "==> Writing ${SEARXNG_DIR}/docker-compose.yml"
backup_if_exists "${SEARXNG_DIR}/docker-compose.yml"
...
services:
  ...
  mcp:
    ...
    ports:
      - "8000:8000"
    environment:
      SEARXNG_ENGINE_API_BASE_URL: "http://core:8001/search"
      MCP_HTTP_HOST: "0.0.0.0"
      MCP_HTTP_PORT: "8000"
    command: ["python", "mcp_server.py", "--http"]
    depends_on:
      - core
...
# ─── .env ───────────────────────────────────────────────────────────────────
echo "==> Writing ${SEARXNG_DIR}/.env"
backup_if_exists "${SEARXNG_DIR}/.env"
cat > "${SEARXNG_DIR}/.env" <<'EOF'
SEARXNG_PORT=8001
EOF

# ─── .env.example ───────────────────────────────────────────────────────────
echo "==> Writing ${SEARXNG_DIR}/.env.example"
backup_if_exists "${SEARXNG_DIR}/.env.example"
cat > "${SEARXNG_DIR}/.env.example" <<'EOF'
# Read the documentation before using the `docker-compose.yml` file:
# https://docs.searxng.org/admin/installation-docker.html
#
# Additional ENVs:
...
echo "==> Writing ${SEARXNG_DIR}/core-config/settings.yml"
backup_if_exists "${SEARXNG_DIR}/core-config/settings.yml"
cat > "${SEARXNG_DIR}/core-config/settings.yml" <<EOF
...
echo "==> Writing ${SEARXNG_DIR}/mcp-server/requirements.txt"
backup_if_exists "${SEARXNG_DIR}/mcp-server/requirements.txt"
cat > "${SEARXNG_DIR}/mcp-server/requirements.txt" <<'EOF'
fastmcp>=3.0.0,<4.0.0
httpx>=0.20.0,<1.0.0
...
echo "==> Writing ${SEARXNG_DIR}/mcp-server/Dockerfile"
backup_if_exists "${SEARXNG_DIR}/mcp-server/Dockerfile"
cat > "${SEARXNG_DIR}/mcp-server/Dockerfile" <<'EOF'
...
echo "==> Writing ${SEARXNG_DIR}/mcp-server/mcp_server.py"
backup_if_exists "${SEARXNG_DIR}/mcp-server/mcp_server.py"
cat > "${SEARXNG_DIR}/mcp-server/mcp_server.py" <<'MCPEOF'
#!/usr/bin/env python3
"""
...
@_fastmcp.tool(
    description="Scrape content from web pages (using Trafilatura, converting Reddit to old.reddit). Caches results and provides citations. Is able to read PDF files and convert to Markdown."
)
...
echo "==> Enabling searxng-stack.service"
systemctl enable searxng-stack.service

echo "==> Starting searxng-stack.service"
if systemctl start searxng-stack.service; then
    ...
    systemctl disable searxng-stack.service --now 2>/dev/null || true
    docker compose -f "${SEARXNG_DIR}/docker-compose.yml" up -d --build
    echo "==> Compose stack started directly. Re-enable the systemd unit on a systemd host."
fi

echo "==> Setup complete."
echo "    SearXNG web UI : http://localhost:8001"
echo "    MCP HTTP server: http://localhost:8000/mcp"
